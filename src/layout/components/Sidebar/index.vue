<template>
  <div>
    <div class="menu_button" :class="[menu_open ? '' : 'mini-menu']"></div>
    <div class="sidebar">
      <ul class="metismenu nav" id="sidebarMenu">
        <li v-for="(nav, index) in menus.sidebars" :key="index">
          <a :href="nav.link">
            <i class="fa" :class="['fa-' + nav.icon]" aria-hidden="true"></i>
            <span>{{nav.name}}</span>
          </a>
          <ul class="nav" v-if="nav.sub_menus">
            <li v-for="(sub, subindex) in nav.sub_menus" :key="subindex">
              <a :href="sub.link" class="on click-on">
                <span>{{sub.name}}</span>
                <i class="fa fa-angle-up" v-if="sub.sub_menus"></i>
              </a>
              <ul class="nav sub-nav" v-if="sub.sub_menus">
