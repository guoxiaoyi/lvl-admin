      ...
      <div class="panel-body">
        <div class="page_toolbar">
          <el-form ref="filterForm" :inline="true" size="small" class="filter-form-inline" @keyup.enter.native="crud.toQuery()">
            ...
            <el-form-item v-if="$route.name !== 'AntiFakes'" label="活动类别" prop="type">
        ...
        <div class="panel panel-default">
          ...
          <el-table v-loading="crud.loading" :data="crud.data" @selection-change="crud.selectionChangeHandler">
            ...
            <el-table-column label="操作" width="140px">
              <template slot-scope="scope">
                <activity-link :to="{ type: scope.row.type, name: 'ActivityShow', params: { activityId: scope.row.id }}" :custom-style="{ marginRight: '10px' }">详情</activity-link>
...
export default {
  ...
  async activated() {
    ...
    await activities.pageType().then(({ data }) => {
      this.pageKind = data
    })
    await activities.typeKind().then(({ data }) => {
      this.typeKind = data
    })
    if (this.$route.name === 'AntiFakes') {
