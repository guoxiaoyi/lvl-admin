...
export default {
  ...
  cruds() {
    return CRUD({ title: '库存管理', url: `/lmp/v2/admin/goods/${this.parent.$route.params.id}/stock_change_list`, props: { pagination: 'concat' }})
  },
  async mounted() {
  ...
  methods: {
    ...
    async clear() {
      if (confirm('确认清空库存？')) {
        this.clearing = true
        store_goods.clear_stock_change({ goodsId: this.$route.params.id }).then(response => {
          window.location.reload()
        }).catch(_err => {
          this.clearing = false
        })
