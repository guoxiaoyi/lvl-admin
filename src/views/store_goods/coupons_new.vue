...
export default {
  ...
  async mounted() {
    ...
    await store_goods.show({ id: this.$route.params.id }).then(response => {
  ...
  methods: {
    submit() {
      store_goods.add_coupon({ goodsId: this.$route.params.id, ...this.form }).then(response => {
        this.$router.push({ name: 'StoreGoodCoupon', id: this.$route.params.id })
      })
    }
  }
}
