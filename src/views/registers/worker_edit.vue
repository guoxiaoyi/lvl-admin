  ...
  <div>
    ...
    <div class="panel panel-default">
      <div class="panel-body">
        <el-form v-if="!loading" ref="form" size="small" label-width="16.6666%" :rules="rules" :model="user">
          <el-form-item label="所属门店">
            <el-input v-model="user.channel.name" disabled />
          </el-form-item>
          ...
          <el-form-item v-for="item in get_custom_fields" :key="item.value" :label="find_custom_field(item.value)['label']">
            <div class="el-input el-input--small is-disabled">
              <input
                v-if="find_custom_field(item.value)['c_kind'] !== 'picture'"
                :value="find_custom_field(item.value)['c_value']"
                type="text"
                autocomplete="off"
