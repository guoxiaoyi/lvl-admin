<template>
  <div class="panel panel-default new-show">
    ...
    <div class="panel-footer">
      <router-link v-if="checkPer(['activity_update'])" :to="{ name: $activityRouterName(detail.type, 'ActivityAdvanceEdit'), params: { ...$route.params }}" class="el-button el-button--success">
  ...
  methods: {
    checkFuncPer(name) {
      return {
        ...
        add_wework_required: this.checkPer(['wework_manage', 'wework_read']) && this.account.wework && !['AntiFakeActivity'].includes(this.detail.type),
