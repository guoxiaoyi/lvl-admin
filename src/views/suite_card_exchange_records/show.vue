      ...
      <div class="panel-body" style="padding-bottom:unset;">
        <div class="well flex">
          ...
          <div style="flex: 1 1 0%;">
            <el-steps v-if="detail.stepInfo" :active="detail.stepInfo.active" align-center finish-status="success">
...
import suite_card_orders from '@/api/suite_card_order.js'
...
import SuiteCardDetail from '@/components/Orders/SuiteCardDetail.vue'
...
export default {
  ...
  methods: {
    submit() {
      this.status = 1
      suite_card_orders.note({ code: this.$route.params.id, ...this.form }).then(({ data }) => {
        this.status = 0
    ...
    deliver() {
      ...
      suite_card_orders[action]({ ...this.deliverModule.form }).then(response => {
    ...
    confirm() {
      if (confirm('请确认订单信息无误，确认接收订单后无法取消。')) {
        suite_card_orders.confirm({ code: this.detail.code }).then(response => {
          window.location.reload()
        })
      }
    },
